...
localparam MAX_PTR   = WORD_FIFO_DEPTH - 1;
...
logic [FIFO_DATA_WIDTH - 1:0] fifo_mem [0: WORD_FIFO_DEPTH - 1];
  ...
  if ( ~rstn_i ) begin
    ...
    if ( flush_i ) write_ptr <= PTR_WIDTH'('b0);
      ...
      if ( write_ptr < MAX_PTR ) begin
        ...
        write_ptr            <= PTR_WIDTH'('b0);
        write_ptr_circle     <= ~write_ptr_circle;
      end
    end
  end
end
